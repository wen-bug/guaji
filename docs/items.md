...
本文档描述物品、背包、装备、炼丹、种田和商店的系统规则。核心物品以 `resources/items/index.tres` 及其 90 个 `ItemDef` 为准，核心技能以 `resources/skills/index.tres` 为准；配方和经济表继续独立保存在 `DataTables`。
...
- 装备实例优先使用自身 `description_effects`；旧存档缺失时，本体装备从独立 `.tres` 配置回填。
- `description_effects` 是可选的新增字段，加载旧档时由背包实例规范化补齐空数组，不单独提升存档 schema；若以后改变字段含义或删除字段，则必须增加 schema 迁移。
- 未支持的 `kind` 默认不显示；新增类型时必须扩展渲染器、本文字段表，并通过调试沙盒和 Godot MCP 验证。
- 渲染器使用 `push_color()`，不解析物品名称或描述中的 BBCode，避免特殊字符破坏格式。
- 当前装备尚无通用“公式伤害”结算入口；在真实 `effects` 或属性结算逻辑接通前，元素伤害公式只能用于开发验证，不能作为已生效的正式装备效果发布。

## 物品分类

掉落物品可通过 `drop_rarity` 标记五档稀有度。现有本体敌人按 normal、elite、boss 使用互不重叠的类别池；旧阶级 `drop_categories`、模板物品列表和显式 `drops` 继续兼容旧存档。同次击杀中所有来源命中的同一 `item_id` 只发放一次。
...
- 丹方不再需要图纸或学习流程：炼丹面板直接列出全部配方，已解锁的可制作，未解锁的置灰并提示所需炼丹建筑等级。
- 丹方随炼丹建筑等级自动解锁；`ALCHEMY_RECIPE_DEFS` 定义 21 个丹方的产物和材料，按建筑 1-7 级逐级开放（完整表见[内容数据表](item-table.md)）。旧档残留的丹方物品在 Schema 23 迁移中按回收价双倍补偿灵石后移除。
...
正式新档拥有 `spirit_stone x1`、`herb x1`，以及雷击术、蚀骨毒雾、回春术、燃锋诀、玄甲术技能书各 1 本。版本 9 迁移会为旧存档补齐尚未持有且尚未学会的技能书，不会重复发放。技能书不进入敌人掉落、商店或生产经济；其余测试物资仍由项目设置 `game/development/seed_test_inventory` 显式开启，默认关闭。
...
账号按 `farm`、`forge`、`alchemy` 保存永久 `output_quality`。可使用物品通过 `building_quality` 类型化效果的 `reference_id` 与 `amount` 提升指定建筑；目标必须是生产建筑且数量必须为正，成功后消耗 1 个物品。当前只有炼器接入实际效果：每点永久品质增加 5% 的单次升阶概率，总概率最高 95%；农田和炼丹仅保留存档与查询接口。

## 规划：装备 V2 与五行技能扩展

本节只描述尚未实现的装备 V2 物品和五行技能书。当前功法残页、寒潮符技能书和丹药规则见[内容数据表](item-table.md)，五行技能规划见[五行技能](skills.md)。

### 成长材料（已实现）

- `enhancement_stone` 强化石由装备分解产出：`阶位数字 + floor(强化点 / 2)` 个；已装备物品禁止分解。
- `ascension_stone` 升阶石由 Boss 必掉获得；重复图纸分解入口已随图纸系统一并退出。
- 强化石只用于装备强化，升阶石只用于装备升阶，均不可直接使用。两石的产量与消耗细则见[装备成长 V2](equipment-progression-v2.md)。
...
- 货架每轮 6 格，所有商品参与轮换，同轮 item_id 唯一且每格限购一次。商品权重为基础材料 45、生产材料 30、丹药 20、稀有 5。
