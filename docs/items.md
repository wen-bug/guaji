...
- 十阶装备等级、升阶、图纸打造、掉落等级、分解经济与旧档迁移采用独立 V2 规划，详见 `docs/equipment-progression-v2.md`；该文档尚未实现，不能替代本节已实现规则。
- 强化失败率、词条锁定、保底机制和高阶灵石合成暂未实现。
- 五行主题装备只提供一个固定主属性，不使用命中、闪避、暴击等额外属性，也不规划职业套装加成；具体目录见 `docs/equipment-progression-v2.md`。
...
- 炼丹建筑 6 级起基础产出翻倍，并按建筑等级每级提供 2% 的额外出丹概率。
...
本节内容均为规划，不代表当前物品类型、掉落、兑换或丹药结算已经实现。规划物品的完整 ID、用途与配方见 `docs/item-table.md`，五行技能见 `docs/skills.md`。
...
- `manual_fragment` 秘法残页是可堆叠、不可直接使用的技能兑换材料。
- 每完成 5 场有效胜利固定获得 1 页，计数不受普通物品和装备掉落结果影响；训练木桩等 `use_drop=false` 的战斗不计数。
- 每个规划五行技能对应一本 `skill_book_<skill_id>` 技能书，玩家定向选择兑换目标，不随机决定技能。
- 四个技能阶段依次消耗秘法残页 `3/6/10/15` 和对应五行灵石 `1/2/3/5`。
- 技能书仍沿用当前永久学习规则：成功学习后消耗，已经学会时不消耗；五行只定义技能特色，不限制跨五行学习或混搭。

### 规划丹药

- 归元丹恢复使用者 25% 最大生命，聚灵丹恢复使用者 25% 最大法力。
- 破军丹、玄甲丹分别令攻击、防御提高 3 点，持续 3 个使用者自身回合。
- 青木丹、赤焰丹、厚土丹、玄金丹、玄水丹令对应五行提高 3 点，持续 3 个使用者自身回合。
- 恢复丹配方为对应生命或法力作物 2 份加草药 1 份。
- 属性丹和五行丹配方为对应作物 2 份、草药 1 份、对应普通或五行灵石 1 份。
- 持续 Buff 丹药共享 `buff_pill` 冷却组，使用后 3 个自身回合内不能再次使用同组丹药；同 ID 状态再次生效时覆盖剩余时间，不叠层。
...
- 林狼独立掉落草药 55%、矿石 30%、灵石 10%、装备 5%，并关闭阶级池；显式掉落和阶级池分别结算。
- 元素普通怪额外掉落对应作物 20%、五行灵石 8%；精英为 40% 和 15%。普通/精英/Boss 装备率分别为 5%/12%/25%。
- 六类装备图纸永久解锁定向打造。完整奖励遭遇胜利有 10% 图纸概率，10 胜保底；全解锁后重复图纸可出现，使用后转换为矿石 4。
- 随机炼器和定向打造均消耗矿石 4。未装备装备可确认后分解，五阶返还 1/2/4/7/12 矿石；已装备物品禁止分解，强化和洗练材料不返还。
