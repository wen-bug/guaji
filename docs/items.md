...
- 五行职业装备只提供一个固定主属性，不使用命中、闪避、暴击等额外属性，也不规划职业套装加成；具体目录见 `docs/equipment-progression-v2.md`。
...
本节内容均为规划，不代表当前物品类型、掉落、兑换或丹药结算已经实现。规划物品的完整 ID、用途与配方见 `docs/item-table.md`，职业技能见 `docs/skills.md`。
...
- 每个规划职业技能对应一本 `skill_book_<skill_id>` 技能书，玩家定向选择兑换目标，不随机决定技能。
- 四个技能阶段依次消耗秘法残页 `3/6/10/15` 和对应五行灵石 `1/2/3/5`。
- 技能书仍沿用当前永久学习规则：成功学习后消耗，已经学会时不消耗；五行职业只是构筑推荐，不限制跨职业学习。
